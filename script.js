let texts = [
	"Bine ai venit",
	"Inca putin si vei fi conectat",
	"Momentan iti cautam masinile",
	"Acum cautam proprietatile pe care le detii",
	"Te rugam sa astepti inca putin",
	"Iti incarcam caracterul",
	"Pregatim jucatorii pentru sosirea ta",
	"Intra si pe discord.gg/ugro",
	"Inca putin si e gata asteptarea",
	"Pentru orice problema, politia te asteapta la sectie",
	"Comunitatea noastra este una din cele mai vechi, inca din 2017",
	"Fondatorul EzK0mG impreuna cu developerul Hyp3 se ocupa de toate problemele",
	"Staff-ul este mereu activ si te poate ajuta cu orice problema",
];
...
$(document).ready(function () {
	...
	// sets texts to h2 here randomly and change after 2 seconds	
	setTimeout(function () {
		// $("#header").show();
		setInterval(function () {
			if (txt >= texts.length) { txt = 0; }
			$("#content h2").fadeOut(function() {
				$(this).text(texts[txt]);
...
var canvas = document.querySelector('canvas'),
    ...
    frequency = .075,
    currentMarker = 0,
    visibleDuration = 6000, // lets us have a shorter invisble period than visible
...
let createDots = () => {
  for (var i = 0; i <= startDotCount; i++) { 
    let dot = new Dot({
      ctx: ctx,
      color: !!Math.floor(Math.random() * 4) ? [153, 51, 102] : [0, 102, 255],
      startTime: window.performance.now(),
      frequency: frequency,
      maxAmplitude: amplitude,
      maxSize: Math.random() * 25,
